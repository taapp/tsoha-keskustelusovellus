...
def get_list():
    ...
    for area in areas_list:
        ...
        area[-1] = (
            "Ei viestejä"
            if area[-1] is None
            else area[-1].strftime("%Y-%m-%d %H:%M:%S")
        )
    return areas_list
...
def create(area_name):
    ...
    if user_id == 0:
        return False
    sql = """INSERT INTO areas (user_id, name, created_at, is_secret, is_visible)
        VALUES (:user_id, :area_name, NOW(), FALSE, TRUE)"""
...
def create_secret(area_name, user_id_list):
    ...
    if user_id == 0:
        return False
    sql = """INSERT INTO areas (user_id, name, created_at, is_secret, is_visible)
        VALUES (:user_id, :area_name, NOW(), TRUE, TRUE)"""
    db.session.execute(sql, {"user_id": user_id, "area_name": area_name})
    db.session.commit()
    ...
    for user_id in user_id_list:
        ...
        db.session.execute(
            sql_users_areas, {"user_id": int(user_id), "area_id": area_id}
        )
    db.session.commit()
    return True
