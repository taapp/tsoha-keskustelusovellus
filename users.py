...
def register(username, password, is_admin):
    ...
    try:
        sql = """INSERT INTO users (name, password, created_at, is_admin, is_visible)
            VALUES (:name, :password, NOW(), :is_admin, TRUE)"""
        db.session.execute(
            sql, {"name": username, "password": hash_value, "is_admin": is_admin}
        )
        db.session.commit()
    except:
        return False
